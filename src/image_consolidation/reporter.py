...
def _short_path(path: str, depth: int = 3) -> str:
    """Return the last `depth` path components prefixed with …/ for compact display."""
...
def generate_dup_review(
    ...
    """
    Write a human-readable Markdown report listing duplicate groups with full
    EXIF data for each file, so the user can spot-check selection choices.

    sort_by: 'size' | 'count' | 'suspicious'
      size       — biggest groups by wasted bytes first (default)
      count      — most copies per group first
      suspicious — groups where winner/loser scores are nearest (least confident choices first)
    """
    total_groups = db.stats()["duplicate_groups"]
    groups = db.review_groups(limit=limit, sort_by=sort_by)
    ...
    now = datetime.now(timezone.utc)
    ts = now.strftime("%Y%m%d_%H%M%S")
    md_path = report_dir / f"dup_review_{ts}.md"

    sort_label = {"size": "wasted space", "count": "file count", "suspicious": "score gap (least confident first)"}.get(sort_by, sort_by)

    lines: list[str] = [
        "# Duplicate Review Report",
        "",
        f"> Generated: {now.strftime('%Y-%m-%d %H:%M:%S UTC')}  ",
        f"> Showing **{len(groups):,}** of **{total_groups:,}** duplicate groups — sorted by {sort_label}  ",
        "> **✓ KEPT** = winner selected by the pipeline  |  **✗ dup** = duplicate that will be removed",
        "",
        "---",
        "",
    ]
    ...
    for g in groups:
        dup_type = "near-dup" if g["is_near_dup"] else "exact"
        gap_note = f" · score gap {g['score_gap']:.3f}" if g["score_gap"] is not None else ""
        lines.append(
            f"### Group {g['group_id']} · {g['file_count']} files"
            f" · {_fmt_bytes(g['total_bytes'] or 0)} total · {dup_type}{gap_note}"
        )
        lines.append("")
        lines.append("| | Path | Fmt | Dimensions | Size | Date | Camera | Score |")
        lines.append("|:--|:--|:--|--:|--:|:--|:--|--:|")

        for f in g["files"]:
            role  = "**✓ KEPT**" if f["is_best"] else "✗ dup"
            path  = _short_path(f["path"])
            fmt   = f["format"] or "—"
            dims  = _dims(f.get("width"), f.get("height"))
            size  = _fmt_bytes(f["size"] or 0)
            date  = (f["exif_date"] or "—")[:10]
            cam   = _camera(f.get("exif_make"), f.get("exif_model"))
            score = f"{f['score']:.4f}" if f["score"] is not None else "—"
            lines.append(
                f"| {role} | `{path}` | {fmt} | {dims} | {size} | {date} | {cam} | {score} |"
            )
    ...
    lines.append("_Report generated by [image-consolidation](https://github.com/your/repo)_")
    lines.append("")

    md_path.write_text("\n".join(lines), encoding="utf-8")
    console.print(f"\n[green]Duplicate review written to:[/green] {md_path}")
    return md_path
